...
public class Humain extends Joueur {
	...
	public Carte[] choisirCarteAJouer() {
		...
		while (nb > nombreMaxCarteJouable) {
			System.out.println("Erreur : Vous ne pouvez jouer au maximum que "
					+ nombreMaxCarteJouable
					+ " carte(s)\nChoisissez un nombre inférieur ou égal à "
					+ nombreMaxCarteJouable);
			nb = scanner.nextInt();
		}
		...
		while (numCarte > 3
			...
			System.out.println("Choix incorrect, vous aviez choisi de jouer "
					+ nb + " carte(s). Recommencez : \n");
			System.out.println(this.mainJoueur);
			numCarte = scanner.nextInt();
		}
		...
		// numCarte=scanner.nextInt();
		// while(numCarte>3){
		// System.out.println("\nIl n'y a que quatre cartes ! Choississez la carte "
		// +i + ": \n");
		// numCarte=scanner.nextInt();
		// }
		// for(int j=0; j<i; j++){
		// if(mainJoueur.getCarte(numCarte).equals(listeCartes[j])) error=true;
		// if(mainJoueur.getCarte(numCarte).getValeur()!=listeCartes[j].getValeur())
		// error =true;
		// }
		// }
		//
